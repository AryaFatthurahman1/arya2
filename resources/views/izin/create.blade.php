...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Buat Pengajuan Izin</h1>
    <p class="text-gray-600 mt-1">Ajukan izin sakit, cuti, atau izin khusus</p>
</div>

<div class="card max-w-2xl">
    <div class="p-6">
        <form method="POST" action="{{ route('leaves.store') }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <select id="karyawan_id" name="karyawan_id" required class="input-field mt-1">
                        ...
                        @foreach(\App\Models\Karyawan::orderBy('nama_lengkap')->get() as $k)
                            <option value="{{ $k->id }}" {{ old('karyawan_id') == $k->id ? 'selected' : '' }}>{{ $k->nama_lengkap }} ({{ $k->nik }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="jenis_izin" name="jenis_izin" required class="input-field mt-1">
                        @foreach(['sakit'=>'Sakit','cuti'=>'Cuti','izin_khusus'=>'Izin Khusus'] as $key => $label)
                            <option value="{{ $key }}" {{ old('jenis_izin') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input id="tanggal_mulai" name="tanggal_mulai" type="date" required class="input-field mt-1" value="{{ old('tanggal_mulai') }}">
                        @error('tanggal_mulai')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="tanggal_selesai" class="block text-sm font-medium text-gray-700">Tanggal Selesai</label>
                        <input id="tanggal_selesai" name="tanggal_selesai" type="date" required class="input-field mt-1" value="{{ old('tanggal_selesai') }}">
                ...
                <div>
                    ...
                    <textarea id="alasan" name="alasan" rows="3" required class="input-field mt-1">{{ old('alasan') }}</textarea>
