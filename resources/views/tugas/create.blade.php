...
<div class="mb-6">
    ...
    <p class="text-gray-600 mt-1">Tugaskan tugas kepada karyawan</p>
</div>
        ...
        <form method="POST" action="{{ route('tasks.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input id="judul" name="judul" type="text" required class="input-field mt-1" value="{{ old('judul') }}">
                    @error('judul')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="deskripsi" class="block text-sm font-medium text-gray-700">Deskripsi</label>
                    <textarea id="deskripsi" name="deskripsi" rows="4" class="input-field mt-1">{{ old('deskripsi') }}</textarea>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="tanggal_tenggat" name="tanggal_tenggat" type="date" required class="input-field mt-1" value="{{ old('tanggal_tenggat') }}">
